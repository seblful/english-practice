...
uv run --project .. english-practice mobile-content
...
```bash
uv run --project .. english-practice mobile-content
# -> mobile/src/practice/content/english_practice.db
```
...
to ship this package unpacked; `practice/storage.py` copes either way, copying
the database out on first launch when it has to.
...
main.py                     builds the dependencies, hands them to the shell
  │
  ├── practice/ui/          app.py (shell) + one module per screen
  │     └── page.py         the slice of ft.Page the screens depend on
  ├── practice/services.py  the one object every screen is handed
  ├── practice/llm.py       three provider adapters over httpx
  ├── practice/providers.py providers, thinking levels, and their translation
  ├── practice/config.py    settings, and the JSON file they live in
  ├── practice/stats.py     one row per graded answer, in SQLite
  ├── practice/storage.py   where files live; unpacking the bundled book
...
- **No LLM SDK.** LangChain's dependency tree does not fit in an APK, and the
  useful part of it here is five HTTP calls. `practice/llm.py` is those calls.
  Each adapter returns its request as data, which is what lets a test assert
  exactly what a given setting sends.
- **No message history.** There is nothing to store, and nothing to trim.

## Working on it

```bash
uv run pytest        # 296 tests, 95% coverage gate
uv run ty check
