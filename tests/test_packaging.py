...
from english_practice.errors import ContentError
from english_practice.packaging import (
    MAX_IMAGE_WIDTH,
...
class TestBuildMobileContent:
    def test_it_copies_every_content_row(self, source_db: Path, tmp_path: Path) -> None:
        ...
        build_mobile_content(source_db, target, SCHEMA_PATH)
    ...
    def test_the_images_are_re_encoded(self, source_db: Path, tmp_path: Path) -> None:
        ...
        result = build_mobile_content(source_db, target, SCHEMA_PATH)
    ...
    def test_it_reports_progress_when_asked(
        ...
        build_mobile_content(
            source_db, tmp_path / "content.db", SCHEMA_PATH, progress=lines.append
        )
    ...
    def test_the_summary_names_the_file_and_the_saving(
        ...
        result = build_mobile_content(source_db, tmp_path / "content.db", SCHEMA_PATH)
    ...
    def test_a_missing_source_says_how_to_build_one(self, tmp_path: Path) -> None:
        with pytest.raises(ContentError, match=r"populate\.py"):
            build_mobile_content(
                tmp_path / "absent.db", tmp_path / "out.db", SCHEMA_PATH
            )

    def test_a_missing_schema_is_reported(
        self, source_db: Path, tmp_path: Path
    ) -> None:
        with pytest.raises(ContentError, match="No schema"):
            build_mobile_content(
                source_db, tmp_path / "out.db", tmp_path / "absent.sql"
            )
    ...
    def test_an_empty_source_produces_an_empty_bundle(self, tmp_path: Path) -> None:
        ...
        result = build_mobile_content(empty, target, SCHEMA_PATH)
