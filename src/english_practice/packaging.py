...
Nothing else changes: same schema, same rows, same ids, so the app runs the
shared queries against it unmodified.
"""
...
def build_mobile_content(
    ...
    *,
    ...
    Args:
        source_path: The bot's database, as ``populate.py`` built it.
        target_path: Where to write the bundle. Overwritten if it exists, and
            its directory is created.
        schema_path: ``scripts/database/schema.sql``, so the bundle is built
            from the same schema as the original rather than a copy of it.
    ...
    Raises:
        ContentError: If the source database or the schema is missing.
    """
    if not source_path.exists():
        raise ContentError(
            f"No exercise database at {source_path}. "
            "Run `uv run scripts/database/populate.py` first."
        )
    if not schema_path.exists():
        raise ContentError(f"No schema at {schema_path}.")
