...
Every stage is resumable: each writes to `data/`, and skips what it already
did, so a run interrupted at unit 180 continues rather than starting over.
Where each stage reads and writes comes from `PathSettings` in
`practice-runtime` — the same layout the bot resolves its database from, so the
file this builds is by construction the file the bot opens.

| Stage | Reads | Writes |
| :---- | :---- | :----- |
| `cut-pdf` | `data/source/murphy.pdf` | `data/source/snippets/` |
| `separate-page-images` | the source PDF | `data/source/images/{grammar,exercises}/` |
| `ocr-grammar-images` | grammar page images | `data/content/grammar/*.md` |
| `organize-exercises` | exercise page images | `data/content/exercises/<unit>/` |
| `extract-answers` | crops + `answers.json` | `data/content/metadata/answers_full.json` |
| `extract-rules` | crops + grammar markdown | `data/content/metadata/rules.json` |
| `populate` | everything above | `data/content/english_practice.db` |
| `bundle` | that database | `packages/app/src/practice_app/content/` |
